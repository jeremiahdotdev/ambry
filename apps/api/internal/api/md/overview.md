...
API access is rate limited per user account with Upstash Redis: 60 requests per minute and 5,000 requests per day across all keys on the account. Requests over the limit return 429 Too Many Requests with Retry-After: 60.
